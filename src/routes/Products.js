...
const Products = () => {
  ...
  //Display results
  const displayResults = data.map((el) => {
    return (
      <div
        key={el.id}
        className="flex flex-col w-[350px] justify-center items-center mb-10 shadow-2xl m-4 rounded-2xl bg-gray-100"
      >
        <div className="w-[300px] flex flex-col justify-center items-center text-wrap px-4 pt-4 mt-4">
          <img src={el.image} alt="card" className="w-[200px] h-[180px] mb-4" />
    ...
    <>
      <div className="flex flex-col justify-center items-center mt-12">
        ...
        <div className="flex flex-row flex-wrap gap-10 mt-10 mb-10 text-gray-700 text-[19px] leading-8 font-medium">
          <button className="border border-solid border-black px-4 py-1  rounded-md">
            All
          </button>
          <button className="border border-solid border-black px-4 py-1 rounded-md">
            Men's Clothes
        ...
        <div className="flex flex-row flex-wrap p-8 justify-center items-center">
          {displayResults}
        </div>
      </div>
    </>
